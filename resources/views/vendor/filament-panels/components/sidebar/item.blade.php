...
<li
    ...
    {{
        $attributes->class([
            ...
            'swm-sidebar-parent' => $hasChildren,
        ])
    }}
>
    <{{ $tag }}
        ...
        @if ($hasChildren)
            {{
                \Filament\Support\generate_icon_html(\Filament\Support\Icons\Heroicon::ChevronDown, attributes: (new \Illuminate\View\ComponentAttributeBag([
                    'x-show' => $sidebarCollapsible && (! $subNavigation) ? '$store.sidebar.isOpen' : true,
                    'x-bind:class' => "{ 'rotate-180': expanded }",
                ]))->class(['fi-sidebar-item-collapse-icon', 'transition-transform']), size: \Filament\Support\Enums\IconSize::Small)
            }}
        @endif
    </{{ $tag }}>

    @if ($hasChildren)
        <ul x-show="expanded" x-collapse.duration.200ms class="fi-sidebar-sub-group-items">
